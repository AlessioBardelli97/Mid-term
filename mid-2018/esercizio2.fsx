...
type Nodo() as this = 
    ...
    let mutable cdg = LWCompositoreDiGrafo()
    ...
    let mutable cur = 50.f

    do
        timer.Tick.Add(fun _ -> 
            let dx = (position.X - boundingBox.Location.X) / cur;
            let dy = (position.Y - boundingBox.Location.Y) / cur;

            boundingBox.Location <- PointF(boundingBox.Location.X + dx, boundingBox.Location.Y + dy)
...
and Arco() = 
    ...
    let mutable text = "empty"
    let mutable lunghezza = 0.f

    member this.Lunghezza
        with get() = lunghezza
        and set(v) = lunghezza <- v
...
and LWButton(g:LWGrafo,cdg:LWCompositoreDiGrafo) as this = 
    inherit LWCControl()
...
and LWGrafo(cdg:LWCompositoreDiGrafo) as this = 
    ...
    let updateText (s:string) =
        ...
        match ar with
            | Some arco -> 
                ...
                cdg.Invalidate()
            | None -> ()
    let pitagora (nodo1:Nodo) (nodo2:Nodo) =
        let c1 = nodo2.Location.X - nodo1.Location.X
        let c2 = nodo2.Location.Y - nodo1.Location.Y
        let q1 = c1 * c1
        let q2 = c2 * c2
        Math.Sqrt(float(q1 + q2)) |> float32
    
    do
        this.Size <- SizeF(1000.f, 1000.f)
    ...
    override this.OnMouseDoubleClick(e) =
        arrayNodi.Add(Nodo(Size = SizeF(50.f,50.f), Location = PointF(single e.Location.X-25.f, single e.Location.Y-25.f), Position = PointF(single e.Location.X-25.f, single e.Location.Y-25.f), CDG = cdg))
    ...
    override this.OnMouseDown(e) =
        ...
        match (isSelezionato,isCtrl) with
            | (Some nodo1,true) -> 
                ...
                match n2 with
                    | Some nodo2 -> 
                        let arco = Arco(Nodo1 = nodo1, Nodo2 = nodo2, Lunghezza = pitagora nodo1 nodo2)
                        arrayArchi.Add(arco)
                        nodo1.Add(arco)
                        nodo2.Add(arco)
                        isSelezionato <- None
    ...
    override this.OnMouseUp(e) = 
        match dragNodo with
            | Some(nodo,_,_) ->
                if nodo.ArrayArchi.Count = 0 then
                    nodo.Position <- nodo.Location
                else
                    let notCanMove = nodo.ArrayArchi |> Seq.tryFind(fun a -> a.Lunghezza <> pitagora a.Nodo1 a.Nodo2)
                    match notCanMove with
                        | Some(_) -> 
                                nodo.Timer.Start()
                        | None -> nodo.Position <- nodo.Location
            | None -> ()
        dragNodo <- None
